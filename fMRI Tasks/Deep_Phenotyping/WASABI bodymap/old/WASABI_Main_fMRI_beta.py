...
nonstimtrialTime = 20 # trial time in seconds (ISI)
#############
...
BodySiteInstructionText = visual.TextStim(win, name='BodySiteInstructionText', 
    text="Experimenter: Please place thermodes on the designated body-sites.",
    font = 'Arial',
...
# Four Conditions: Hot1, Hot2, Warm1, Warm2, Rest
...
# 2 Conditions: Imagine1 Imagine2
...
# Initialize components for Routine "OverallRating"
...
for thisrunLoop in runLoop:                     # Loop through each run.
    ...
    while continueRoutine:
        ...
        if BodySiteImg1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            BodySiteImg1.tStart = t  # local t and not account for scr refresh
            BodySiteImg1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(BodySiteImg2, 'tStartRefresh')  # time at next scr refresh
            BodySiteImg1.setAutoDraw(True)
        ...
        if BodySiteImg2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            BodySiteImg2.tStart = t  # local t and not account for scr refresh
            BodySiteImg2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(BodySiteImg2, 'tStartRefresh')  # time at next scr refresh
            BodySiteImg2.setAutoDraw(True)
    ...
    runLoop.addData('BodySiteImg1.started', BodySiteImg1.tStartRefresh)
    runLoop.addData('BodySiteImg1.stopped', BodySiteImg1.tStopRefresh)
    runLoop.addData('BodySiteImg2.started', BodySiteImg2.tStartRefresh)
    runLoop.addData('BodySiteImg2.stopped', BodySiteImg2.tStopRefresh)
    # check responses
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                if fix_cross.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    ...
                    if trial_type == 1:
                        ...
                        win.callOnFlip(print, "Starting Warm Stimulation to the", bodySites[runLoop.thisTrialN][0])
                    elif trial_type == 4:
                        win.callOnFlip(print, "Starting Warm Stimulation to the", bodySites[runLoop.thisTrialN][1])
                    elif trial_type == 7:
        ...
        ## Nonstimulus Trials
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                if NonStimTrialText.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > NonStimTrialText.tStartRefresh + nonstimtrialTime-frameTolerance:
                        # keep track of stop time/frame for later
