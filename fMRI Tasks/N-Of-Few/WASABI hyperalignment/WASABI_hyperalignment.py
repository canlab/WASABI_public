...
#     # set up handler to look after randomisation of conditions etc
    ...
    for thisTrialLoop in trialLoop:
        if eyetracker_exists==1:
            sourceEDF_filename = "S%dR%s.EDF" % (int(expInfo['subject number']), trialLoop.thisTrialN+1)
            destinationEDF = os.path.join(sub_dir, "S%dR%s.EDF" % (int(expInfo['subject number']), trialLoop.thisTrialN+1))
