...
for runs in runRange:
    ...
    if runs==6:
        ...
        if expInfo['second(2) or third(3) day']=='2':
            ...
            if runs==4 or runs==5:
                # temperature = 44 # Not hot enough
                temperature = float(participant_settingsHeat[bodySites[runs]])-1
                thermodeCommand=thermode_temp2program[temperature]
                ConditionName="Placebo"
                # Warm and not hot temperature, set to -1 degrees from max for everyone

            # Below for 2.0.3 settings
        ...
        # Changes in 2.0.5: Increase uncertainty from ses-02 by setting all trials to HighHeat-.5 instead of HighHeat-1.
        elif expInfo['second(2) or third(3) day']=='3':
            if runs in [0, 1, 6, 7]:
                ConditionName="Control"
            if runs in [2, 3, 4 ,5]:
                ConditionName="Placebo"
            temperature = participant_settingsHeat[bodySites[runs]]-.5 # 2.0.5 Revision 1/18/2023. Temperature-.5.
            thermodeCommand=thermode_temp2program[temperature]
