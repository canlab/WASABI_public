...
This paradigm is run as 2 parts of a MedMap paradigm for 2 body sites: Chest and Left Leg. 

One day will consist of 8 total runs of either Chest or Left Leg interchanging between each run. Each run will feature 6+1(dummy) trials of varying degrees of heat C. 
...
Version 2.0.4: Revised the stimulation temperatures to reflect a higher and more consistent contrast between placebo and control on ses-02. Ses-03 temperatures revised to high-1 to generate more uncertainty from ses-02 than was present previously.
Version 2.0.5: Originally revised stimulation temperatures are insufficiently painful. Revised the stimulation temperatures of placebo on ses-02 to reflect .5 degree steps-down. Ses-03 temperatures revised to high-.5.
Version 2.0.6: Previous versions do not produce requisite placebo effect in expectations or rated intensities, and at test (Day 3), the temperature is not matched with the distractmap (high). Revised the design to maximize temperature conditioning distance for the longest time like the Paingen study (high vs. low (44.5)).
...
for runs in runRange:
    ...
    if runs==6:
        ...
        # Changes in 2.0.6: maximize difference between control(HighHeat) and placebo (44.5) for the entirety of the conditioning phase. 
        if expInfo['second(2) or third(3) day']=='2':
            if runs in [0, 1, 6, 7]:
            ...
            if runs in [2, 3, 4, 5]:
                ...
                # temperature = float(participant_settingsHeat[bodySites[runs]])-.5
                temperature = 44.5
                thermodeCommand=thermode_temp2program[temperature]
                ConditionName="Placebo"
                # Warm and not hot temperature, set to -1 degrees from max for everyone

        # Changes in 2.0.6: Test at high-heat to match distractmap.
        elif expInfo['second(2) or third(3) day']=='3':
            if runs in [0, 1, 6, 7]:
                ConditionName="Control"
            if runs in [2, 3, 4 ,5]:
                ConditionName="Placebo"
            temperature = participant_settingsHeat[bodySites[runs]] # 2.0.6 Revision 3/11/2023. High Temperature.
            thermodeCommand=thermode_temp2program[temperature]
