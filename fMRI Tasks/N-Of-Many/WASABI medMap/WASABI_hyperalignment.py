...
The paradigm will generate 1x of these files of name:
sub-SIDXXXXXX_task-hyperalignment_run-X_events.tsv

1x trials per file with the following
...
IMPORT Participant information from Parent Script
"""
...
""" 
3. Setup the Window
IMPORT THE WINDOW FROM PARENT SCRIPT
...
#     os.makedirs(sub_dir)
# psypy_filename = sub_dir + os.sep + u'sub-SID%06d_ses-%02d_task-%s_%s' % (int(expInfo['DBIC Number']), int(expInfo['session']), expName, expInfo['date'])
...
### Possible implementations of run
# def run(movie_filenames, *args):
#     # args is a tuple
#     for a in args:
#         print(a)
    
#     varargin = args
#     nargin = 1 + len(varargin) # 1 mandatory argument (movie_filenames) 
    

# def run(movie_filenames, **kwargs):
...
def run(movie_filenames, biopac_exists, biopac_info):

    # Should be contained in biopac_info
    # Biopac_info should be an OrderedDict or some sort of list I can find keys for.
    # extract from biopac_info
    biopac_exists=biopac_info['biopac_exists']
    biopac=biopac_info['biopac']

    for code in range(len(biopac_info)):
        task_ID=4
        hyperalignment_intro = 212
    ...
    movieOrder=[OrderedDict([('runSeq', [])])]
    movie_biopac_codes=[]
    for moviefile in movie_filenames:
        movie = visual.MovieStim3(
            win=win, name=os.path.split(moviefile)[1],
            noAudio = False,
            filename=moviefile,
            ori=0, pos=(0, 0), opacity=1,
            loop=False,
        ...
        movies.append(movie)
        movieOrder[0]['runSeq'].append(OrderedDict([('moviefile', moviefile)]))
        inscapes=214
        movie_biopac_codes.append(biopac_info['moviefile'])
    ...
    # INHERIT globalClock and routineTimer from Parent
    ...
    """
    # Start demarcation of the hyperalignment task in Biopac Acqknowledge
    if biopac_exists:
    ...
    #     # set up handler to look after randomisation of conditions etc
        ...
        for thisTrialLoop in trialLoop:
            ...
            9. Save Run File
            """
            bids_run_filename = sub_dir + os.sep + u'sub-SID%06d_ses-%02d_task-%s_run-%s_events.tsv' % (int(expInfo['DBIC Number']), int(expInfo['session']), expName, str(trialLoop.thisTrialN+1))
