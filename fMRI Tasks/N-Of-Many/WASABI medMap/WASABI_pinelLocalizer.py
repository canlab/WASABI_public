...
# SETUP DEVICES -- SHOULD INHERIT FROM PARENT
# if test == 1:
#     winSize = testWinSize
#     fullScr = True
#     screen = 0
...
def run():
    ...
    stim_dir = os.path.join(base_dir, 'pinel_stim')
    ...
    # create file w/subId in name
    # change this so that filename is from teh gui- and needs extension. so if not . in
    fName = 'sub-SID%06d_task-pinel_ses-%02d.tsv' % (int(expInfo['DBIC Number']),int(session))
    ...
    for item in vis_subtraction_pre:  # ex calc1, calc2
        for word in range(len(item)):  # for i in 0-3
            item[word] = visual.TextStim(win=win, text=item[word],
    ...
    ho_check1 = visual.ImageStim(win, image=ho_pic1, name=os.path.split(ho_pic1)[-1],
                                pos=(0.0, 0.0), width=100, height=100)  # this stim changes too much for autologging to be useful
    ho_check2 = visual.ImageStim(win, image=ho_pic2, name=os.path.split(ho_pic2)[-1],
                                pos=(0.0, 0.0), width=100, height=100)  # this stim changes too much for autologging to be useful
    vert_check1 = visual.ImageStim(win, image=vert_pic1, name=os.path.split(vert_pic1)[-1],
                                pos=(0.0, 0.0), width=100, height=100)  # this stim changes too much for autologging to be useful
    vert_check2 = visual.ImageStim(win, image=vert_pic2, name=os.path.split(vert_pic2)[-1],
                                pos=(0.0, 0.0), width=100, height=100)  # this stim changes too much for autologging to be useful
    ...
    wait_stim = visual.TextStim(win=win, name='waitText',
    ...
    if biopac_exists:
        ...
        win.callOnFlip(biopac.setData, biopac, task_ID)
        win.callOnFlip(biopac.setData, biopac, 0)
    win.flip()
    ...
    for key, task in task_order.items():
        ...
        if biopac_exists:
            win.callOnFlip(biopac.setData, biopac, 0)
            win.callOnFlip(biopac.setData, biopac, word2biopaccode[key])
        ...
        if "checker" in key:
            ...
            if biopac_exists:
                win.callOnFlip(biopac.setData, biopac, 0)
                win.callOnFlip(biopac.setData, biopac, word2biopaccode['fixation'])
            win.flip()
    ...
    if biopac_exists:
        win.callOnFlip(biopac.setData, biopac, 0)
        win.callOnFlip(biopac.setData, biopac, word2biopaccode['end'])
    win.flip()
