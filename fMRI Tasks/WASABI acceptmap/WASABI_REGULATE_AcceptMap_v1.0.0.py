...
This paradigm is run as part 2 of the AcceptanceMap paradigm for each of 8 body sites. 
This paradigm is meant to be run between a pre-regulation experience phase and a post-regulation experience phase, which are found in:
WASABI_EXPERIENCE_AcceptMap_v1.0.0.py

This paradigm is a simple instruction to follow regulation instructions that come from a prior training, 
a repeating series of fixation crosses will coincide with a hot stimulation, followed by an intensity rating.
Following a set number of trials, the subject will be asked a series of questions regarding the run.

As a consequence, correct running of these paradigms will generate 8x files of the names:
...
# Initialize components for Routine "RegulateInstructions"
RegulateInstructionsClock = core.Clock()
RegulateInstructions = visual.TextStim(win=win, name='RegulateInstructions',
...
for r in range(totalTrials): # 8 repetitions
    ...
    if r == 0:      # First trial
        ...
        acceptmap_bids_trial.extend((onset, globalClock.getTime() - fmriStart - onset, None, bodySites, temperature, "Regulation Instruction", None))
    ...
    acceptmap_bids_trial.extend((onset, t, None, bodySites, temperature, "Regulation Phase", jitter1))
