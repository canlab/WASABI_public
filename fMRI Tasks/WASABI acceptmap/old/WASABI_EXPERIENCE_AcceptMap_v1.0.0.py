...
This paradigm is run as part 1 and part 3 of the AcceptanceMap paradigm for each of 8 body sites. 
This paradigm is meant to be run twice. Once as a pre-regulation experience phase, once as a post-regulation experience phase. One run is designed to be sandwiched in between as
WASABI_REGULATE_AcceptMap_v1.0.0.py

This paradigm is a simple instruction to experience the stimuli as they come, followed by a repeating of a fixation crosses that coincide with a hot stimulation and an intensity rating.
Following a set number of trials, the subject will be asked a series of questions regarding the run.

As a consequence, correct running of these paradigms will generate 16x files of the names:
8x sub-XXXXX_ses-XX_task-acceptmap-pre-exp_acq-XXXX_run-X_events.tsv
8x sub-XXXXX_ses-XX_task-acceptmap-post-exp_acq-XXXX_run-X_events.tsv
...
# Set this later
# task_ID=8   # If pre-experience
# task_ID=10  # If post-experience
...
expName = 'acceptMap_experience'  # from the Builder filename that created this script
if debug == 1:
    expInfo = {
    'subject number': '99',
    'gender': 'm',
    'acceptMap body-site # (day1: 1-4; day2: 5-8)': '1', 
    'acceptMap (1)pre- or (2)post-Experience (1 or 2)': '1',
    'session': '99',
    'handedness': 'r', 
    'scanner': 'MS',
    'run': '1'
    ...
    if dlg1!=None:
        if "_task-Calibration_participants.tsv" in dlg1[0]:
            ...
            if a.shape == (1,39):
                ...
                expInfo['acceptMap (1)pre- or (2)post-Experience (1 or 2)'] = expInfo2['acceptMap (1)pre- or (2)post-Experience (1 or 2)']
            ...
            dlg1=None
    if dlg1==None:
        dlg2 = gui.DlgFromDict(title="WASABI AcceptMap Experience Scan", dictionary=expInfo, sortKeys=False)
        if dlg2.OK == False:
            core.quit()  # user pressed cancel
...
if expInfo['acceptMap (1)pre- or (2)post-Experience (1 or 2)'] == '1':
    ...
    task_ID=8
if expInfo['acceptMap (1)pre- or (2)post-Experience (1 or 2)'] == '2':
    expName = 'acceptmap-post-exp' 
    task_ID=10
...
# Initialize components for Routine "ExperienceInstructions"
ExperienceInstructionsClock = core.Clock()
ExperienceInstructions = visual.TextStim(win=win, name='ExperienceInstructions',
...
"""
# ExperienceInstructions.setText("Text")
ExperienceInstructions.draw()
win.flip()
...
while continueRoutine:
    ...
    if BodySiteImg.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        BodySiteImg.image = bodysite_word2img[bodySites]
        BodySiteImg.pos = (0, .2)
        # keep track of start time/frame for later
...
10. Start Trial Loop
"""
for r in range(totalTrials): # 8 repetitions
    """
    11. Show the Instructions prior to the First Trial
    """
    if r == 0:      # First trial
        ExperienceInstructions.setText("Experience the following sensations as they come.")
        ExperienceInstructions.draw()
        if biopac_exists:
            biopac.setData(biopac, 0)
            biopac.setData(biopac, experience_instructions)
        ...
        if expInfo['acceptMap (1)pre- or (2)post-Experience (1 or 2)'] == '1':
            acceptmap_bids_trial.extend((onset, globalClock.getTime() - fmriStart - onset, None, bodySites, temperature, "Pre-Regulated Experience Instruction", None))
        if expInfo['acceptMap (1)pre- or (2)post-Experience (1 or 2)'] == '2':
            acceptmap_bids_trial.extend((onset, globalClock.getTime() - fmriStart - onset, None, bodySites, temperature, "Post-Regulated Experience Instruction", None))
    ...
    if expInfo['acceptMap (1)pre- or (2)post-Experience (1 or 2)'] == '1':
        acceptmap_bids_trial.extend((onset, t, None, bodySites, temperature, "Pre-Regulated Experience Phase", jitter1))
    if expInfo['acceptMap (1)pre- or (2)post-Experience (1 or 2)'] == '2':
        acceptmap_bids_trial.extend((onset, t, None, bodySites, temperature, "Post-Regulated Experience Phase", jitter1))
    ...
    acceptmap_bids_trial = []
    acceptmap_bids_trial.extend((onset, t, sliderValue, bodySites, temperature, "Intensity Rating", jitter2))
    acceptmap_bids.append(acceptmap_bids_trial)
...
acceptmap_bids.append(["Pain Boredomance Rating:", sliderValue])
...
acceptmap_bids_filename = sub_dir + os.sep + u'sub-%05d_ses-%02d_task-%s_acq-%s_run-%s_events.tsv' % (int(expInfo['subject number']), int(expInfo['session']), expName, bodySites.replace(" ", "").lower(), int(expInfo['run']))
