...
Design 3.0.0 of the Distractmap was generated because in 2.0.0, we randomly switched between 1-back and 2-back conditions. This led to a large switching cost, which nullified any pain perceptual differences between the two conditions.
To increase the difference in the demand between the two conditions, we are now contrasting Rest with 2-back, taking care to generate randomized mini-blocked design in order to minimize the switching cost and allow the working memory/distraction effect to have maximal impact on pain perception.

The 8-body-site test will now be reduced to 2-body-sites, in an ABAB design, akin to the AcceptMap
    ...
    if dlg1!=None:
        if "_task-Calibration_participants.tsv" in dlg1[0]:
            ...
            if a.shape == (1,39):
                ...
                if ~use_calibration_temps:
                    expInfo['bodysite1 temp']=expInfo2['bodysite1 temp']
                    expInfo['bodysite2 temp']=expInfo2['bodysite2 temp']
...
# Initialize components for Routines "Rest"
RestClock = core.Clock()
fixation_rest = visual.TextStim(win=win, name='fixation_rest',
    text='+',
    font='Arial',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0, 
...
sliderMin = -.75
sliderMax = .75
intensityText = "How intense was the heat overall?"
...
# Deprecated as of 3.0.0, now only doing two-back vs. rest
...
NbackInstructionText2 = "During the task you will be presented a white square in one of nine positions on a grid. \n\n\n\n\n\n\n\n\nYour task is to indicate whether the \ncurrent position is the same as the position two trials ago\n\n\nExperimenter press [Space] to continue."
...
NbackInstructionText4 = "\n2-back\n\n\n\n\n\nDuring the task you will have to indicate whether the current position matches the position that was presented two trials ago, by either pressing the \"yes\" button (left click) or the \"no\" button (right click).\n\n\nExperimenter press [Space] to see an example."
    ...
    for thisComponent in FixationComponents:
        ...
        for thisTrial_2 in trials_2:
            ...
            if gotValidClick==True and (response_2.click_left == 1 or response_2.click_right == 1):  # we had a response
                trials_2.addData('response_2.rt_left', response_2.rt_left)
                trials_2.addData('response_2.rt_right', response_2.rt_right)
            ...
            if cheat == 1:
                ...
                score = correct*100/trials_2.nTotal
...
# 1. 1x Rest - Fixation Cross, no-stimulation                       [3 trials]
# 2. 2x Rest - Fixation Cross, high-heat stimulation (~49 degrees)  [6 trials]
...
for runs in range(len(bodySites)):
    ...
    # 06292022: Deprecated with version 3.0.0
...
#     thermodeCommand = thermode1_temp2program[participant_settingsHeat[bodySites[runs]]]  
    ...
    temperature1 = expInfo['bodysite1 temp']
    thermodeCommand1 = thermode1_temp2program[expInfo['bodysite1 temp']]  
    temperature2 = expInfo['bodysite2 temp']
    thermodeCommand2 = thermode1_temp2program[expInfo['bodysite2 temp']]

    if runs==0 or runs==2:
        temperature = temperature1
        thermodeCommand = thermodeCommand1
    if runs==1 or runs==3:
    ...
    for r in BlockTrials: # 6 blocks
        if r==1 or r==2:
            ...
            for trial_in_mini_block in range(trials_per_block):
                trigger=0 # Set trigger
                """ 
                14. Begin Condition 1: Rest Fixation-Cross No-Stimulation or Condition 3: Rest Fixation-Cross Stimulation 
                """
                ...
                while continueRoutine and routineTimer.getTime() > 0:
                    ...
                    if r==2:
                        ...
                        if trigger==0 and routineTimer.getTime()<stimtrialTime-4 and thermode_exists == 1:
                    ...
                    if fixation_rest.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                        ...
                        if biopac_exists:
                            win.callOnFlip(biopac.setData, biopac, 0)
                            if r==1:
                                win.callOnFlip(biopac.setData, biopac, rest_nostim)
                            if r==2:
                                win.callOnFlip(biopac.setData, biopac, rest_stim)
                            fixation_rest.setAutoDraw(True)
                    if fixation_rest.status == STARTED:
                        # is it time to stop? (based on global clock, using actual start)
                        if tThisFlipGlobal > fixation_rest.tStartRefresh + stimtrialTime-frameTolerance:
                            # keep track of stop time/frame for later
                ...
                for thisComponent in RestComponents:
                    if hasattr(thisComponent, "setAutoDraw"):
                        thisComponent.setAutoDraw(False)
                thisExp.addData('fixation_rest.started', fixation_rest.tStartRefresh)
                thisExp.addData('fixation_rest.stopped', fixation_rest.tStopRefresh)

                ## I think how this works is it will do 3 unjittered rest stimulation trials in the block...
                ## Probably the same with 2-back, I will want to jitter those as well.
                
                distractmap_bids_trial = []
                
                if r == 1:
                    distractmap_bids_trial.extend((onset, t, None, None, None, bodySites[runs], 32, "Rest-NoStim", jitter1))
                if r == 2:
                    distractmap_bids_trial.extend((onset, t, None, None, None, bodySites[runs], temperature, "Rest-Stim", jitter1))
                distractmap_bids.append(distractmap_bids_trial)
                ...
                """ 
                14iv. Post-Trial Fixation Cross
                """
                # ------Prepare to start Routine "Fixation"-------
                continueRoutine = True
                ...
                distractmap_bids_trial = []
                distractmap_bids_trial.extend((onset, t, None, None, None, bodySites[runs], None, "PostTrialFixation", None))
                distractmap_bids.append(distractmap_bids_trial)
                ...
                if r==2:
                    ...
                    distractmap_bids_trial = []
                    distractmap_bids_trial.extend((onset, t, None, sliderValue, None, bodySites[runs], temperature, "Rest-Stim Rating", jitter2))
                    distractmap_bids.append(distractmap_bids_trial)
        ...
        if r==3 or r==4:
            for trial_in_mini_block in range(trials_per_block):
                ...
                if r==3:
                    ...
                    distractmap_bids_trial.extend((onset, globalClock.getTime() - fmriStart, None, None, None, bodySites[runs], 32, "2back Instructions", None))
                distractmap_bids.append(distractmap_bids_trial)
                ...
                for thisTrial_2 in trials_2:
                    ...
                    # ------Prepare to start Routine "N_back_2_trials"-------
                    # Trigger Thermal Program
                    if r==4 and trials_2.thisTrialN == 4 and thermode_exists == 1:
                        sendCommand('trigger')
                    ...
                    if r==3:
                        if trials_2.thisTrialN == 1:
                            distractmap_bids_trial.extend((onset, t, mouse_response_rt, mouse_response, response_2.corr, bodySites[runs], 32, "2back-nostim", jitter1))
                        else:
                            distractmap_bids_trial.extend((onset, t, mouse_response_rt, mouse_response, response_2.corr, bodySites[runs], 32, "2back-nostim", None))
                    if r==4:
                        if trials_2.thisTrialN == 1:
                            distractmap_bids_trial.extend((onset, t, mouse_response_rt, mouse_response, response_2.corr, bodySites[runs], temperature, "2back-stim", jitter1))
                        else:
                            distractmap_bids_trial.extend((onset, t, mouse_response_rt, mouse_response, response_2.corr, bodySites[runs], temperature, "2back-stim", None))
                    distractmap_bids.append(distractmap_bids_trial)
                ...
                # keep track of which components have finished
                ScoreReportComponents = [ScoreReportText]
                for thisComponent in ScoreReportComponents:
                    thisComponent.tStart = None
                    thisComponent.tStop = None
                    thisComponent.tStartRefresh = None
                ...
                while continueRoutine:
                    ...
                    if ScoreReportText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                        ...
                        if biopac_exists:
                            ...
                            win.callOnFlip(biopac.setData, biopac, nback_feedback)
                        ScoreReportText.setAutoDraw(True)
                    if ScoreReportText.status == STARTED:
                        # is it time to stop? (based on global clock, using actual start)
                        if tThisFlipGlobal > ScoreReportText.tStartRefresh + jitter2-frameTolerance:
                            # keep track of stop time/frame for later
                            ScoreReportText.tStop = t  # not accounting for scr refresh
                            ScoreReportText.frameNStop = frameN  # exact frame index
                            win.timeOnFlip(ScoreReportText, 'tStopRefresh')  # time at next scr refresh
                            ScoreReportText.setAutoDraw(False)
                ...
                if r==4:
                    ...
                    distractmap_bids_trial = []
                    distractmap_bids_trial.extend((onset, t, None, sliderValue, None, bodySites[runs], temperature, "2back-stim Rating", jitter2))
                    distractmap_bids.append(distractmap_bids_trial)
