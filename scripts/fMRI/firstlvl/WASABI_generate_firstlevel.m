...
function WASABI_genfirstlvlDSGNs(fmriprep_derivdir, bidsroot, spike_def, omit_spike_trials, spikes_percent_threshold, vif_thresh, movement_reg_quadratic, subjs2analyze, dvars_threshold, spike_additional_vols, pmod_polynom, pmod_name, pmod_ortho_off, pmod_type, plotdesign, plotmontages, input_threshold, thresh_type, k, mask, varargin)
    ...
    % MANDATORY REQUIRED
    mandatory.spike_def = 'fMRIprep';
    % mandatory.spike_def = 'CANlab';
    mandatory.omit_spike_trials = 'no';
    mandatory.spikes_percent_threshold=0.15;
    mandatory.vif_thresh=2;
    mandatory.movement_reg_quadratic = true; % change to false if you don't want to add quadratic terms for movement parameters and their first-order derivatives
    ...
    % SPIKE OPTIONS
    spikeopts.dvars_threshold = 2; % REQUIRED if spike_def = 'CANlab'
    spikeopts.spike_additional_vols=0; % OPTIONAL, NOT RECOMMENDED TO TURN ON
    ...
    if strcmpi(spike_def,'CANlab')==1 && ~isfield(spikeopts,'dvars_threshold')
        error('spike_def option %s requires specification of spikeopts.dvars_threshold, please specify before proceeding', mandatory.spike_def)
    end
